...
그리고 제가 사용하는 태그라는 것이...? 플러그인이 모두 설치된 도구를 말하시는 건가요? 음... 그런 버전은 없어 제공이 어려울 듯 합니다.
...
> 과정 스케줄 표를 보면 HTML/CSS를 공부하는 과정(워드프레스 테마 수정을 위한)인데 워드프레스가 주인공인 과정으로 변질된 듯 합니다. 물론 수업 때 워드프레스를 다루긴 하지만, 워드프레스가 『주』이고 HTML/CSS가 『보조』인 듯 이야기 주셨네요. (안타깝게도 마음에 들지 않는 수업을 진행하고 있어 죄송합니다) 참고로 테마를 변경하여 레이아웃을 바꾸기 위해서는 HTML/CSS/Javascript 작성 능력이 있어야 합니다. 제공되는 무료 또는 유료 테마에 전적으로 의지해서는 원하는 디자인을 얻을 수 없습니다. 왜? 1기 분들이 테마를 설치했음에도 불구하고 HTML/CSS 공부가 필요했는지 언젠가는 알게 되겠지요. (훗날...) 그리고 테마를 고르는 방법 또는 유용한 플러그인은 수업 시간에 다루도록 하겠습니다.
...
> 원론적인 HTML 사용법이라 말씀주셨는데.. 원론적이라는 것의 의미는 '근본'을 말하는 것이죠. 맞습니다. 이 과정에서 배우는 것은 그 '근본'을 배우는 것입니다. 실제로 워드프레스를 사용할 때 얼마나 도움이 될 지 모르겠다고 이야기 주셨는데... 워드프레스를 사용해보면 어떤 도움이 될지 알게 되겠지요.
...
> 주석으로 넘기다는 것이 무엇을 의미하는 지는 정확히 알 수 없으나, 아마도 주석 처리하지 말고 그냥 놔두라는 것으로 판단됩니다. 가급적 HTML/CSS 수업 중 주석 처리를 하지 않도록 하겠습니다. 혹시 주석 처리를 하게 되면 의견을 주시길 바랍니다. ^ㅡ^
